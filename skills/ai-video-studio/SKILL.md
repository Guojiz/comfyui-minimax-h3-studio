...
- **开始使用（第一次需要连接或生成）时必须先问客户“这次怎么接入”**：连接网络上的
  ComfyUI、在自己电脑上部署的 ComfyUI、还是通过本地自定义节点接入第三方 API Key。
  三种方式都通过同一个 MCP 连接 ComfyUI，但实例来源与外部服务授权不同，不能默认选择，
  也不能因为之前用过某一种就自动沿用。
...
三种接入方式统一走同一个 MCP：连接远端 ComfyUI、本机部署的 ComfyUI、或本地自定义节点
接入第三方 API Key，最终都是通过 `mcp/comfyui_mcp.py` 连接一个 ComfyUI 实例。差别只在
catalog 里登记哪个地址、以及该实例的本地节点是否带第三方适配。开始使用时先按客户选择的
方式登记/选择实例并写入项目锁，再进入下面的任务流程。
